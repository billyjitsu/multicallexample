...
describe("Multi", function () {
  async function deployContracts() {
    ...
    const MultiCallRead = await ethers.getContractFactory("Multicall3");
    const multiCallRead = await MultiCall.deploy();
  ...
  describe("Deployment", function () {
    it("Read the functions timestamps", async function () {
      const { testContract, multiCall, owner } = await loadFixture(
        deployContracts
      );
  ...
  describe("MultiCallTest", function () {
    it("Test with MultiCall", async function () {
      const { testContract, multiCall, owner } = await loadFixture(
        deployContracts
      );

      // const callActions = ["func1", "func2", "func3", "func4"]; // Function names to be called
      const callActions = ["func1"]; // Function names to be called
      ...
      try {
        // const returnData = await multiCall.aggregate3(calls);
         const returnData = await multiCall.staticAggregate3(calls);
